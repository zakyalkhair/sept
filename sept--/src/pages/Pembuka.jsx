...
export default function Pembuka() {
  ...
  return (
    <PageShell className="relative grid min-h-dvh place-items-center overflow-hidden p-6">
      ...
      {/* Judul + video dalam SATU kolom dengan `gap` tetap — jaraknya disetarakan
          dengan margin kiri-kanan video (86vw → sisa 7vw per sisi). Dulu
          keduanya digeser terpisah pakai vh, jadi jaraknya ikut melar di
          layar yang tinggi. */}
      <div className="absolute z-20 flex -translate-y-[4vh] flex-col items-center gap-[min(7vw,2.5rem)]">
      <motion.h1
        data-kursor="judul"
        initial={{ opacity: 0 }}
        animate={{ opacity: formasiSelesai ? 1 : 0 }}
        transition={{ duration: DUR.enter, ease: EASE.in }}
        className="h-display select-none text-center text-[clamp(2.2rem,9vw,5.5rem)]"
      >
